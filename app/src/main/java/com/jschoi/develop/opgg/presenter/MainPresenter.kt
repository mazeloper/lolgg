...
class MainPresenter(private var view: MainContract.View, private var context: Context) :
    ...
    private fun matchDataParse(item: MatchDTO) {
        ...
        // 승패확인
        win = if (item.participants[searchUserIndex].stats.win) {
            "승리"
        } else {
            "패배"
        }
        ...
        championImageUrl = "${Config.CHAMPION_INFO_URL}${championImage["full"]}"
        ...
        spell1 = "${Config.SPELL_IMAGE_URL}${IntroActivity.spellList[spell1Id]?.get("id")}.png"
        spell2 = "${Config.SPELL_IMAGE_URL}${IntroActivity.spellList[spell2Id]?.get("id")}.png"
        // TODO 메인 룬 이미지
